...
var CMDRunImport = cli.Command{
	Name:   "import",
	Usage:  "import the record_process",
	Action: RunImport,
	Flags: []cli.Flag{
		cli.StringFlag{
			EnvVar: "DORMON_QQ_RECORD",
			Name:   "record_process, F",
			Usage:  "specify the configuration file",
			Value:  "./record_process.record_process",
		},
		cli.StringFlag{
		...
		cli.BoolFlag{
			...
			Name:   "overwrite, O",
			Usage:  "overwrite the record_process",
		},
	},
}
...
var CMDRunFix = cli.Command{
	Name:   "fix",
	Usage:  "standardizing the record_process file",
	Action: RunFix,
	Flags: []cli.Flag{
		cli.StringFlag{
			EnvVar: "DORMON_QQ_RECORD",
...
func RunImport(c *cli.Context) {
	...
	content, err := ioutil.ReadFile(c.String("record_process"))
...
func RunFix(c *cli.Context) {
	...
	f, err := os.OpenFile(c.String("record_process"), os.O_RDWR, 0644)
	if err != nil {
		panic(err)
	}
	defer f.Close()
